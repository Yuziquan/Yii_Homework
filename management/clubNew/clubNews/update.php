                ...
                <table class="mt15">
                        ...
                        <td>
                            <span id="club_box">
                                <?php if ($model->club_id != null){ ?>
                                    ...
                                    <?php echo $model->news_club_name; ?>
                                    <?php echo $form->hiddenField($model, 'club_id', array('class' => 'input-text')); ?>
                                    </span><?php } else { ?><span
                                        class="label-box"><?php echo get_session('club_name'); ?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <span id="project_box"><?php foreach ($project_list as $v) { ?><span class="label-box"
                                                                                                 id="project_item_<?php echo $v->project_id; ?>"
                                                                                                 data-id="<?php echo $v->project_id; ?>"><?php if (!empty($v->project_list)) echo $v->project_list->project_name; ?>
                                    <i onclick="fnDeleteProject(this);"></i></span><?php } ?></span>
                        ...
                        <td colspan="3">
                            ...
                            <span id="club_list_box">
                                <?php if (!empty($club_list)) foreach ($club_list as $v) { ?>
                                    <span class="label-box" id="club_item_<?php echo $v->recommend_clubid; ?>"
                                          data-id="<?php echo $v->recommend_clubid; ?>"><?php echo $v->club_list->club_name; ?>
                                        <i onclick="fnDeleteClub(this);"></i></span>
                                <?php } ?>
                            </span>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if ($model->news_pic != '') { ?>
                                <div class="upload_img fl" id="upload_pic_ClubNews_news_pic"><a
                                        href="<?php echo $basepath->F_WWWPATH . $model->news_pic; ?>"
                                        target="_blank"><img
                                            src="<?php echo $basepath->F_WWWPATH . $model->news_pic; ?>"
                                            width="100"></a></div><?php } ?>
                            <input style="margin-left:5px;" id="picture_select_btn" class="btn" type="button"
                                   value="图库选择">
                    ...
                    <tr id='show_pic_line' style="display:none;"><!--news_type=226时显示，此外为多图，链接club_news_pic表-->
                        ...
                        <td colspan="3">
                            ...
                            <div class="upload_img fl" id="upload_pic_club_news_pic">
                                <?php $basepath = BasePath::model()->getPath(165);
                                $picprefix = '';
                                if ($basepath != null) {
                                    $picprefix = $basepath->F_CODENAME;
                                } ?>
                                ...
                                <table id="club_news_pic">
                                    <?php if (!empty($club_news_pic)) foreach ($club_news_pic as $v) { ?>
                                        <tr>
                                            <td width="150"><input type="hidden"
                                                                   name="club_news_pic[<?php echo $v['id']; ?>][id]"
                                                                   value="<?php echo $v['id']; ?>"><input type="hidden"
                                                                                                          name="club_news_pic[<?php echo $v['id']; ?>][pic]"
                                                                                                          value="<?php echo $v['news_pic']; ?>"><a
                                                        class="picbox" data-savepath="<?php echo $v['news_pic']; ?>"
                                                        href="<?php echo $basepath->F_WWWPATH . $v['news_pic']; ?>"
                                                        target="_blank">
                                                    <img src="<?php echo $basepath->F_WWWPATH . $v['news_pic']; ?>"
                                                         width="100">
                                                </a></td>
                                            <td><textarea oninput="LimitText(this)" onpropertychange="LimitText(this)"
                                                          name="club_news_pic[<?php echo $v['id']; ?>][intro]"
                                                          class="input-text" style="width:80%;height:80px;"
                                                          placeholder="请输入图片介绍... 50字以内"><?php echo $v['introduce']; ?></textarea>
                                            </td>
                                            <td width="50"><a class="btn" href="javascript:;" onclick="fnDelPic(this);"
                            ...
                            <script>
                                we.uploadpic('<?php echo get_class($model);?>_club_news_pic', '<?php echo $picprefix;?>', '', '', function (e1, e2) {
                        ...
                        <td colspan="3" style="padding:30px 15px;">
                            ...
                            <div class="c">
                                ...
                                <?php if ($model->gf_material != null) { ?>
                                    <span class="label-box">
                                    <a href="<?php echo $model->gf_material->v_file_path . $model->gf_material->v_name; ?>"
                                       target="_blank">
                                ...
                                <div class="upload fl">
                                    <script>var materialVideoUrl = '<?php echo $this->createUrl('gfMaterial/upvideo');?>';
                                        we.materialVideo(function (data) {
                                            $('#ClubNews_news_video').val(data.id).trigger('blur');
                                            $('#video_box').html('<span class="label-box">' + data.name + '</span>');
                                        }, 61, 24, '上传');</script>
                                </div>
                                <input style="margin-left:5px;" id="video_select_btn" class="btn fl" type="button"
                                       value="选择视频">
                            </div>
